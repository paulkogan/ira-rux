import React, {Component} from 'react';
import {BrowserRouter, Route, Switch, Link, NavLink } from "react-router-dom";
import {formatCurrency} from './ira-utils'

//component get props
const DealsFinancials = (props) => {
  const dealFinancials = props.dealFinancials;
  //const dealLink = "/dealdetails/"+ props.dealResult.id
  return (

            <div>
                  <u> Deal Financials for   {dealFinancials.name} </u><br/>
                   Entity_ID: {props.entityID}  <br/>
                   Deal_ID: {dealFinancials.id}  <br/>
                   total_assets: {formatCurrency(dealFinancials.total_assets)}  <br/>
                   total_debt: {formatCurrency(dealFinancials.total_debt)}  <br/>
                   equity_value: {formatCurrency(dealFinancials.equity_value)}  <br/>
                   notes: {dealFinancials.notes}  <br/>
                  <br/>

            </div>

  )

}
export default DealsFinancials;
